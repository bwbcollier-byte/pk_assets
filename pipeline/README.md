...
| `prompt_template.py` | Deterministic builder for the Prompt Text field. Produces the exact format 21st.dev hands you when you click "Copy prompt": preamble + component code + optional demo + optional tailwind config + optional registry dependency source + NPM install line + implementation guidelines. Called by the harvest and detect scripts — no LLM involved. |
| `generate_fields_batch.py` | For records with code but thin copy, asks **Gemini 2.5 Flash** (free tier) for a JSON blob with `description` and `code_html` only — Prompt Text is templated, not generated. Falls back to **OpenRouter free models** (default `deepseek/deepseek-chat-v3.1:free`) when Gemini keys are exhausted. Both pools are rotated round-robin and pulled from the Logins & Keys base at runtime. Supports `realtime` (default) and `apply <results.jsonl>`. |
